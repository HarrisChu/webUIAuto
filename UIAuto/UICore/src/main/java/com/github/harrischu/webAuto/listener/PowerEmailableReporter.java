package com.github.harrischu.webAuto.listener;
...
import com.github.harrischu.webAuto.util.ConfigReader;
		...
		if (tests.getAllResults().size() > 0) {
			...
			for (ITestNGMethod method : getMethodSet(tests, suite)) {
				...
				buff.append("<td><a href=\"#m" + methodId + "\">" + qualifiedName(method) + " "
						...
						+ parameterString + "</td>"	+ "<td>" + convertDate(start) + "</td>" + "<td class=\"numi\">" + (end - start) + "</td>" + "</tr>");
			}
			if (mq > 0) {
				cq += 1;
				m_out.print("<tr class=\"" + style + (cq % 2 == 0 ? "even" : "odd") + "\">" + "<td");
...
//		}
	...
	protected void startHtml(PrintWriter out) {
		...
		out.println("img {width:60%;height:60%}");
		out.println("</style>");
